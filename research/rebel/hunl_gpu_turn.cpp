...
BatchTurnSolver::BatchTurnSolver(TreeShape shape, std::vector<TurnSpec> specs,
    ...
    for (int b = 0; b < B_; ++b) {
        ...
        // per-runout showdown structures (rows b*52+c). Board-card slots
        // stay all-zero: runout_ok_ kills their opp mass, so every gather
        // lands on zero-padded prefix cells and contributes nothing.
...
void BatchTurnSolver::iterate(int t) {
    ...
    const double w = pcfr_ && pcfr_quad_
